...
<div class="flex flex-col gap justify-center items-center">
  ...
  <div class="flex flex-row gap-2 mt-2">
    <button onclick={pState.isRunning ? stop : start} class="text-white px-3 py-1 bg-gray-500 hover:bg-gray-400 rounded-lg">{pState.isRunning ? 'Stop' : 'Start'}</button>
    <button onclick={switchMode} class="text-white px-3 py-1 bg-gray-500 hover:bg-gray-400 rounded-lg">{pState.mode === 'work' ? 'Break' : 'Work'}</button>
  </div>
</div>