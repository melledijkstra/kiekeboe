<script lang="ts">
  ...
  import { mdiHomeOutline, mdiPlus, mdiPlusCircle, mdiRocketLaunch, mdiSpa } from "@mdi/js"
...
<header class="w-full p-5">
  ...
  <div class="group float-right flex flex-row items-start justify-end align-middle gap-5">
    <div class="relative">
      <IconButton
        icon={mdiPlus}
        class={[
      ...
      <Card class={[
        ...
        "absolute right-0 text-center p-5 w-80",
          ...
          <div class="flex flex-row justify-between gap-4 items-stretch">
            <button onclick={() => metrics = [...metrics, { title: 'Ramadan', counter: 32 }]}>
              <Icon path={mdiPlusCircle} />
              Countdown
            </button>
            <button onclick={() => metrics = [...metrics, { title: 'Dubai', counter: 12 }]}>
              <Icon path={mdiPlusCircle} />
              World Clock
            </button>
            <button onclick={() => metrics = [...metrics, { title: 'Sleep', counter: 8 }]}>
              <Icon path={mdiPlusCircle} />
              Sleep Tracker
            </button>
          </div>
